      ...
      <div>
        <div className="flex flex-col gap-4 justify-center items-center">
          ...
          {value?.map(({ file, progress }, i) => (
            <div
              key={i}
              className="flex h-16 w-96 flex-col justify-center rounded border border-solid border-black/50 p-2"
            >
              <div className="flex items-center gap-2 text-black">
                <FileIcon size="30" className="shrink-0 text-black" />
                ...
                <div className="flex w-12 justify-end text-xs">
                  {progress === "PENDING" ? (
                    <button
                      className="rounded-md border-none bg-transparent p-1 transition-colors duration-200 hover:bg-black/30"
                      onClick={() => {
                        void onChange?.(
                    ...
                    <LucideFileWarning className="shrink-0 text-black-400" />
                  ) : progress !== "COMPLETE" ? (
                    <div>{Math.round(progress)}%</div>
                  ) : (
                    <CheckCircleIcon className="shrink-0 text-green-400" />
                  )}
                </div>
              </div>
              {/* Progress Bar */}
              {typeof progress === "number" && (
                <div className="relative h-0">
                  <div className="absolute top-1 bg-black h-1 w-full overflow-clip rounded-full bg-black/30">
                    <div
